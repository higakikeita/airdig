...
func (c *Client) InitSchema(ctx context.Context, schemaSQL string) error {
	...
	// This is a simplified version - in production you might want to use a migration tool

	// For now, just execute the entire schema
	// Note: ClickHouse doesn't support multi-statement execution in a single Exec call
	// so you'd need to split by semicolons and execute separately
...
func (c *Client) HealthCheck(ctx context.Context) (*HealthStatus, error) {
	...
	// Get drift events count
	var driftCount uint64
	query := "SELECT count() FROM drift_events WHERE date >= today() - INTERVAL 7 DAY"
	if err := c.QueryRow(ctx, query).Scan(&driftCount); err != nil {
		// Table might not exist yet, that's okay
		driftCount = 0
	}
	status.DriftEventsLast7Days = driftCount
...
type HealthStatus struct {
	...
	DriftEventsLast7Days uint64                 `json:"drift_events_last_7_days"`
...
func NewClientFromDSN(dsn string) (*Client, error) {
	...
	if len(options.Addr) > 0 {
		// Parse host:port from first address
